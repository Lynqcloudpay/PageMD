  ...
  try {
    ...
    if (result.rows.length > 0 && !result.rows[0].is_admin && result.rows[0].role_id) {
      try {
        ...
        `, [result.rows[0].role_id, adminPrivileges]);
        ...
        console.warn('Error checking admin privileges in auth middleware:', error.message);
      }
    }
    ...
    // Check if user is active (allow NULL status as active for backward compatibility)
