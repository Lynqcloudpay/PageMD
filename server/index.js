...
// ============================================
// PRODUCTION SAFETY CHECKS - HIPAA COMPLIANCE
// ============================================
if (process.env.NODE_ENV === 'production') {
  // 1. Force audit logging in production
  if (process.env.ENABLE_AUDIT_LOGGING !== 'true') {
    console.error('❌ CRITICAL: ENABLE_AUDIT_LOGGING must be true in production for HIPAA compliance');
    process.exit(1);
  }
  
  // 2. Force PHI encryption in production
  if (process.env.ENABLE_PHI_ENCRYPTION !== 'true') {
    console.error('❌ CRITICAL: ENABLE_PHI_ENCRYPTION must be true in production for HIPAA compliance');
    console.error('   PHI encryption is mandatory. Set ENABLE_PHI_ENCRYPTION=true in your environment.');
    process.exit(1);
  }
  
  // 3. Ensure HTTPS is enforced
  if (process.env.FORCE_HTTPS !== 'true') {
    console.warn('⚠️  WARNING: FORCE_HTTPS should be true in production. Setting it now.');
    process.env.FORCE_HTTPS = 'true';
  }
  
  console.log('✅ Production safety checks passed');
  console.log('   - Audit logging: ENABLED');
  console.log('   - PHI encryption: ENABLED');
  console.log('   - HTTPS enforcement: ENABLED');
}

// Trust proxy for reverse proxy (Caddy/Nginx) - REQUIRED for HTTPS detection
