/**
 * Update User to Physician with Admin Privileges
 * 
 * Usage: node scripts/update-user-to-physician-admin.js <email>
 */
...
async function updateUserToPhysicianAdmin(email) {
  ...
  try {
    ...
    if (userResult.rows.length === 0) {
      ...
      if (similar.rows.length > 0) {
        ...
        similar.rows.forEach(r => console.log(`  - ${r.email}`));
      }
      process.exit(1);
    }
    ...
    // Find Physician role (try different case variations)
    ...
    if (user.role_id === physicianRole.id && user.is_admin === true) {
      console.log(`\n✅ User already has Physician role and admin privileges!`);
...
if (!email) {
  console.error('Usage: node scripts/update-user-to-physician-admin.js <email>');
  console.error('Example: node scripts/update-user-to-physician-admin.js meljrodriguez14@gmail.com');
  process.exit(1);
}

updateUserToPhysicianAdmin(email)
  .then(() => {
    console.log('\n✅ Done!');
    process.exit(0);
  })
