    ...
    if (planStructured.length === 0 && diagnoses.length === 0) {
        return (
            <div className="text-center py-10">
                ...
                <p className="text-sm text-gray-400 font-medium">No plan items yet</p>
                <p className="text-xs text-gray-400 mt-1">Add diagnoses in the Assessment section to begin building the plan.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4">
            {planStructured.map((item, dIdx) => {
                ...
                return (
                    <div key={dIdx} className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:border-primary-100 transition-all shadow-sm">
                        {/* Diagnosis Header */}
                        <div
                            className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-gray-50/50 transition-all"
                            onClick={() => toggleCard(dIdx)}
                        >
                            {!isLocked && <GripVertical className="w-3.5 h-3.5 text-gray-400 cursor-grab shrink-0" />}
                            <div className="flex-1 min-w-0">
                                <h4 className="text-sm font-bold text-gray-800 truncate">{dIdx + 1}. {item.diagnosis}</h4>
                            </div>
                            {orderCount > 0 && (
                            ...
                            {!isLocked && (
                                <button onClick={(e) => { e.stopPropagation(); removeFromPlan(dIdx); }} className="p-1 text-gray-400 hover:text-red-500 transition-all shrink-0" title="Remove diagnosis from plan">
                                ...
                                {item.orders?.length > 0 ? (
                                    <div className="space-y-1.5 mb-3">
                                        {item.orders.map((order, oIdx) => (
                                            <div key={oIdx} className="flex items-center gap-2 py-1.5 px-2 rounded-lg hover:bg-gray-50 group transition-all">
                                                {getOrderIcon(order)}
                                                <span className="text-sm text-gray-600 flex-1">{order}</span>
                                                {!isLocked && (
                                                    <button onClick={() => removeFromPlan(dIdx, oIdx)} className="opacity-0 group-hover:opacity-100 p-0.5 text-gray-400 hover:text-red-500 transition-all">
                                                        <X className="w-3 h-3" />
                                                    </button>
                                                )}
                                            </div>
                                    ...
                                    <p className="text-xs text-gray-400 italic mb-3 py-2">No orders placed for this diagnosis</p>
                                )}
                                        ...
                                        <div className="flex-1 min-w-[140px]">
                                            <div className="flex items-center gap-1">
                                                <input
                                                    ...
                                                    className="flex-1 text-[11px] border-gray-200 rounded-lg px-2 py-1 focus:ring-primary-400 focus:border-primary-400"
                                                />
