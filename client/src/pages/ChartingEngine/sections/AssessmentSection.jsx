        ...
        <div className="space-y-3">
            ...
            {diagnoses.map((dx, idx) => (
                <div
                    ...
                    className={`flex items-center gap-3 p-3 bg-white rounded-xl border transition-all group ${dragIndex === idx ? 'border-primary-400 shadow-lg opacity-70' : 'border-gray-100 hover:border-primary-200'
                        }`}
                >
                    {!isLocked && <GripVertical className="w-3.5 h-3.5 text-gray-400 cursor-grab opacity-0 group-hover:opacity-100 transition-opacity shrink-0" />}
                    <span className="text-sm font-bold text-gray-800 flex-1">{idx + 1}. {dx.replace(/^\d+\.\s*/, '')}</span>
                    {!isLocked && (
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => { setEditingIndex(idx); setShowHierarchy(true); }} className="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-all" title="Change code">
                                <Edit3 className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => removeDiagnosis(idx)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="Remove">
            ...
            {!isLocked && (
                <div className="relative mt-3">
                    <div className="flex items-center gap-2">
                        <div className="relative flex-1" ref={searchRef}>
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-primary-400 focus:ring-2 focus:ring-primary-100 transition-all"
                            />
                        </div>
                    ...
                    {showResults && results.length > 0 && (
                        <div ref={resultsRef} className="absolute z-50 top-full left-0 right-0 mt-1 border border-gray-200 rounded-xl bg-white shadow-xl max-h-64 overflow-y-auto">
                            {results.slice(0, 12).map((r, i) => (
                                <div key={i} className="flex items-center justify-between px-4 py-2.5 hover:bg-primary-50 text-sm border-b border-slate-50 last:border-0 group/item">
                                    <button className="flex-1 text-left" onClick={() => handleSelectCode(r)}>
                                        ...
                                        <span className="text-gray-600 ml-2">{r.description}</span>
                                    </button>
            ...
            {diagnoses.length === 0 && (
                <div className="text-center py-8">
                    ...
                    <p className="text-sm text-gray-400">No diagnoses yet. Search ICD-10 codes above or import from the problem list.</p>
                </div>
            )}
