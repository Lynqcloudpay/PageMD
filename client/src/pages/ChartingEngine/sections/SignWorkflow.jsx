                    ...
                    <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4 shadow-xl">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-500">This note requires cosignature from an attending physician.</p>
                            </div>
                        </div>
                        <select value={selectedAttendingId} onChange={(e) => setSelectedAttendingId(e.target.value)} className="w-full p-2.5 border border-gray-200 rounded-xl mb-4 text-sm focus:ring-primary-400 focus:border-primary-400">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button onClick={() => setShowSignPrompt(false)} className="px-4 py-2.5 text-gray-600 hover:bg-gray-50 rounded-xl text-sm transition-all">Cancel</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
                    <div className="bg-white rounded-2xl p-6 max-w-lg w-full mx-4 shadow-xl">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <button onClick={() => setShowCosignModal(false)} className="p-1 text-gray-400 hover:text-gray-600 rounded-lg hover:bg-gray-50"><X className="w-4 h-4" /></button>
                        </div>

                        {/* Authorship Model */}
                        <div className="mb-4">
                            <label className="text-xs font-bold text-gray-600 uppercase tracking-wide mb-1 block">Authorship Model</label>
                            <div className="flex gap-2">
                                {['Addendum', 'Full Attestation', 'Direct Edit'].map(model => (
                                    <button key={model} onClick={() => { setAuthorshipModel(model); if (model === 'Direct Edit') onSetDirectEditing?.(true); }}
                                        className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition-all ${authorshipModel === model ? 'bg-primary-50 text-primary-600 border-primary-200' : 'bg-white text-gray-600 border-gray-200 hover:border-primary-200'}`}>
                                        {model}
                                    </button>
                                ))}
                            </div>
                        ...
                        {attestationMacros.length > 0 && (
                            <div className="mb-3">
                                <label className="text-xs font-bold text-gray-600 uppercase tracking-wide mb-1 block">Quick Templates</label>
                        ...
                        <textarea value={attestationText} onChange={(e) => setAttestationText(e.target.value)}
                            ...
                            className="w-full border border-gray-200 rounded-xl p-3 text-sm min-h-[100px] focus:ring-primary-400 focus:border-primary-400 mb-4" />
                        ...
                        <div className="flex gap-2">
                            ...
                            <button onClick={() => setShowCosignModal(false)} className="px-4 py-2.5 text-gray-600 hover:bg-gray-50 rounded-xl text-sm">Cancel</button>
                        </div>
                    </div>
                </div>
            )}
                    ...
                    <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4 shadow-xl">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-500">This action cannot be easily undone. The note will be marked as entered in error.</p>
                            </div>
                        </div>
                        <select value={retractReason.reason_code} onChange={(e) => setRetractReason({ ...retractReason, reason_code: e.target.value })} className="w-full p-2 border border-gray-200 rounded-xl mb-3 text-sm">
                        ...
                        <textarea value={retractReason.reason_text} onChange={(e) => setRetractReason({ ...retractReason, reason_text: e.target.value })} placeholder="Additional details..." className="w-full border border-gray-200 rounded-xl p-3 text-sm min-h-[80px] mb-4" />
                        <div className="flex gap-2">
                            ...
                            <button onClick={() => setShowRetractModal(false)} className="px-4 py-2.5 text-gray-600 hover:bg-gray-50 rounded-xl text-sm">Cancel</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {showDeleteConfirm && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
                    <div className="bg-white rounded-2xl p-6 max-w-sm w-full mx-4 shadow-xl text-center">
                        ...
                        <p className="text-sm text-gray-500 mb-4">This will permanently delete this unsigned note.</p>
                        <div className="flex gap-2">
                            ...
                            <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-2.5 text-gray-600 hover:bg-gray-50 rounded-xl text-sm">Cancel</button>
                        </div>
                    </div>
                </div>
            )}
