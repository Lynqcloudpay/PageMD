...
const PendingNotes = () => {
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto">
            {/* Header */}
            <div className="mb-6">
                <h1 className="text-xl font-bold text-[#111827] tracking-tighter uppercase mb-0.5">Pending Notes</h1>
                <p className="text-[11px] text-gray-500 font-medium uppercase tracking-wide">{pendingVisits.length} note{pendingVisits.length !== 1 ? 's' : ''} pending completion or signature</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                >
                    <div className="flex items-center justify-between mb-2">
                        <span className={`text-xs font-semibold uppercase tracking-wider ${filterStatus === 'all' ? 'text-blue-700' : 'text-gray-500'}`}>Total Pending</span>
                        <div className={`p-1.5 rounded-lg ${filterStatus === 'all' ? 'bg-blue-100 text-blue-600' : 'bg-gray-50 text-gray-400 group-hover:bg-blue-50 group-hover:text-blue-500'}`}>
                            <FileText size={16} />
                        </div>
                    </div>
                    <div className="flex items-baseline gap-1">
                        <span className={`text-3xl font-semibold ${filterStatus === 'all' ? 'text-[#2563EB]' : 'text-[#111827]/60'}`}>
                ...
                >
                    <div className="flex items-center justify-between mb-2">
                        <span className={`text-xs font-semibold uppercase tracking-wider ${filterStatus === 'draft' ? 'text-emerald-700' : 'text-gray-500'}`}>Ready to Sign</span>
                        <div className={`p-1.5 rounded-lg ${filterStatus === 'draft' ? 'bg-emerald-100 text-emerald-600' : 'bg-gray-50 text-gray-400 group-hover:bg-emerald-50 group-hover:text-emerald-500'}`}>
                            <CheckCircle2 size={16} />
                        </div>
                    </div>
                    <div className="flex items-baseline gap-1">
                        <span className={`text-3xl font-semibold ${filterStatus === 'draft' ? 'text-emerald-700' : 'text-[#111827]/60'}`}>
                ...
                >
                    <div className="flex items-center justify-between mb-2">
                        <span className={`text-xs font-semibold uppercase tracking-wider ${filterStatus === 'incomplete' ? 'text-amber-700' : 'text-gray-500'}`}>Incomplete</span>
                        <div className={`p-1.5 rounded-lg ${filterStatus === 'incomplete' ? 'bg-amber-100 text-amber-600' : 'bg-gray-50 text-gray-400 group-hover:bg-amber-50 group-hover:text-amber-500'}`}>
                            <AlertCircle size={16} />
                        </div>
                    </div>
                    <div className="flex items-baseline gap-1">
                        <span className={`text-3xl font-semibold ${filterStatus === 'incomplete' ? 'text-amber-700' : 'text-[#111827]/60'}`}>
            ...
            <div className="flex items-center gap-3 mb-6">
                ...
                <div className="flex items-center gap-2">
                    <div className="bg-gray-50 rounded-xl p-1 flex">
                        <button
                            onClick={() => setFilterStatus('all')}
                            className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all ${filterStatus === 'all' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                        >
                            All
                        </button>
                        <button
                            onClick={() => setFilterStatus('draft')}
                            className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all ${filterStatus === 'draft' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                        >
                            Ready
                        </button>
                        <button
                            onClick={() => setFilterStatus('incomplete')}
                            className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all ${filterStatus === 'incomplete' ? 'bg-white text-amber-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                        >
                            Incomplete
                        </button>
                    </div>
                                    ...
                                    {group.items.map((visit) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className="px-5 py-4 hover:bg-gray-50 cursor-pointer transition-colors group relative border-l-[3px] border-l-transparent hover:border-l-blue-500"
                                            >
                                                <div className="flex items-start justify-between gap-4">
                                                    ...
                                                    <div className="flex-1 min-w-0 flex items-start flex-col sm:flex-row sm:items-center gap-2 sm:gap-6">
                                                        ...
                                                        <div className="flex-shrink-0 w-44">
                                                            <div className="font-semibold text-sm text-gray-800 truncate group-hover:text-blue-700 transition-colors">
                                                                {patientName}
                                                            </div>
                                                            <div className="text-xs text-gray-500 font-mono mt-0.5 bg-gray-50 inline-block px-1.5 rounded">
                                                                {visit.mrn}
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="flex-shrink-0 w-40 text-sm text-gray-600">
                                                            <div className="font-medium text-gray-700">{visit.visit_type || 'Office Visit'}</div>
                                                            <div className="flex items-center gap-1.5 text-xs text-gray-500 mt-0.5">
                                                                <Calendar className="w-3 h-3" />
                                                                <span>{format(new Date(visit.visit_date), 'MMM d')}</span>
                                                                {daysSince > 0 && (
                                                                    <span className={`px-1.5 rounded text-[10px] font-medium ${daysSince > 7 ? 'bg-red-50 text-red-600' : daysSince > 3 ? 'bg-orange-50 text-orange-600' : 'bg-gray-50 text-gray-500'}`}>
                                                                        {daysSince}d old
                                                                    </span>
                                                                )}
                                                            </div>
                                                        ...
                                                        <div className="flex-1 min-w-0 text-sm text-gray-600 hidden md:block">
                                                            {notePreview?.hpi ? (
                                                                ...
                                                                <span className="text-gray-400 italic text-xs">No content draft started...</span>
                                                            )}
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="flex items-center gap-4 flex-shrink-0">
                                                        ...
                                                        <div className="p-1 rounded-full text-gray-400 group-hover:text-blue-600 group-hover:bg-blue-50 transition-all">
