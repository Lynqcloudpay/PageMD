    ...
    const refreshPatientData = useCallback(async () => {
        ...
        try {
            ...
            try {
                ...
                setSurgicalHistory(surgicalHistResponse?.data || []);
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                                        ...
                                        <div className="p-2 h-[280px] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200">
                                            {(problems || []).length > 0 ? (
                                                <div className="space-y-0.5">
                                                    {(problems || []).map(prob => (
                                                        <div key={prob.id} className="flex items-center justify-between px-2 py-1.5 rounded-md hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0 group">
                                                            <div className="flex items-center gap-2 min-w-0">
                                                                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 flex-shrink-0" title="Active" />
                                                                <span className="text-[10px] font-bold text-slate-700 truncate group-hover:text-blue-700 leading-tight">{prob.name}</span>
                                                            </div>
                                ...
                                <div className="lg:col-span-3 space-y-5">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                        {/* Medications Module - Full Height */}
                                        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col overflow-hidden hover:border-blue-200 transition-colors h-[416px]">
                                                        ...
                                                        <div className="space-y-1.5">
                                                            {familyHistory.map(hist => (
                                                                <div key={hist.id} className="flex justify-between items-center px-2 py-1.5 rounded-lg bg-slate-50/30 border border-slate-100/50">
                                                                    <span className="text-slate-800 text-[10px] font-bold truncate max-w-[100px]">{hist.condition}</span>
                                                                    <span className="text-slate-400 text-[8px] font-black tracking-tighter bg-white px-1 py-0.5 rounded border border-slate-100">{hist.relationship}</span>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    ) : (
