...
const ClinicPersonnelManager = ({ clinicId, clinicSlug, apiCall, users, setUsers }) => {
    ...
    if (loading) return <div className="text-gray-400 text-sm animate-pulse">Loading personnel...</div>;

    return (
        <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
                <thead className="text-gray-400 border-b border-gray-100 uppercase text-[10px] tracking-widest bg-gray-50/50">
                    <tr>
                ...
                <tbody className="divide-y divide-gray-100">
                    {users.length === 0 ? (
                        <tr><td colSpan="4" className="py-8 text-center text-gray-400 italic">No users found in this clinical schema.</td></tr>
                    ) : (
                        users.map((user) => (
                            <tr key={user.id} className="group hover:bg-gray-50/50 transition-colors">
                                <td className="p-4">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-gray-400 text-[10px] mt-1 uppercase tracking-wider">{user.role_display_name || user.role}</span>
                                    </div>
                                </td>
                                ...
                                <td className="p-4 text-right">
                                    <div className="flex items-center justify-end gap-2">
                                        <button
                                            ...
                                            className="p-2 rounded-lg bg-white border border-gray-200 text-gray-500 hover:text-orange-500 hover:border-orange-200 hover:shadow-sm transition-all"
                                        >
                                        ...
                                        <button
                                            ...
                                            className="p-2 rounded-lg bg-white border border-gray-200 text-gray-500 hover:text-blue-500 hover:border-blue-200 hover:shadow-sm transition-all"
                                        >
                                        ...
                                        <button
                                            ...
                                            className={`p-2 rounded-lg bg-white border border-gray-200 transition-all hover:shadow-sm ${user.status === 'active' ? 'text-gray-500 hover:text-red-500 hover:border-red-200' : 'text-gray-500 hover:text-green-500 hover:border-green-200'
                                                }`}
                                        >
...
const DriftManager = ({ clinicId, apiCall }) => {
    ...
    if (loading) return <div className="text-gray-400 text-sm animate-pulse p-4">Auditing permission schemas...</div>;

    return (
        <div className="space-y-4">
            {drift.map((report) => (
                <div key={report.roleKey} className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h4 className="text-sm font-bold text-gray-800 flex items-center gap-2">
                                    {report.displayName}
                                    <span className={`text-[10px] uppercase px-2 py-0.5 rounded-full font-bold tracking-tighter ${report.status === 'SYNCED' ? 'bg-emerald-500/10 text-emerald-500' :
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${report.status === 'SYNCED' ? 'bg-emerald-50 text-emerald-600 cursor-default' :
                                'bg-white border border-gray-200 text-gray-700 hover:border-indigo-500 hover:text-indigo-600 shadow-sm'
                                }`}
                        >
        ...
        <div className="space-y-6">
            ...
            <div className="p-5 rounded-2xl bg-gradient-to-br from-indigo-600 to-blue-700 text-white shadow-xl shadow-indigo-200/50 relative overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <div className="mb-4 p-3 bg-white/10 rounded-xl border border-white/20 space-y-2">
                        <p className="text-[9px] font-bold text-indigo-100 uppercase tracking-widest">Average Cost Equation</p>
                        <div className="flex flex-col gap-1 text-[11px] font-medium text-white/90">
                            ...
                            <div className="pt-1 mt-1 border-t border-white/20 flex justify-between font-bold">
            ...
            {tiers.length > 0 && (
                <div className="p-4 rounded-xl bg-gray-50 border border-gray-100">
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-3">Tier Breakdown</p>
                            ...
                            return (
                                <div key={i} className={`flex items-center justify-between py-1.5 px-2 rounded-lg transition-all ${isActive ? 'bg-indigo-100 border border-indigo-200' : 'opacity-50'}`}>
                                    <div className="flex items-center gap-2">
                                        <span className={`w-2 h-2 rounded-full ${isActive ? 'bg-indigo-500' : 'bg-gray-200'}`}></span>
                                        <span className={`text-xs font-bold ${isActive ? 'text-indigo-700' : 'text-gray-400'}`}>{tier.name}</span>
                                        <span className="text-[10px] text-gray-400">({tier.min === tier.max ? tier.min : `${tier.min}-${tier.max}`} seats)</span>
                                    </div>
                                    <span className={`text-xs font-bold ${isActive ? 'text-indigo-600' : 'text-gray-400'}`}>${tier.rate}/seat</span>
                                </div>
                            );
                        })}
                    </div>
                        ...
                        {referrals?.length > 0 ? referrals.map((ref, i) => (
                            <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                                <td className="px-4 py-3">
                                    <div className="font-bold text-gray-800">{ref.referred_clinic_name || 'Prospect'}</div>
                                    <div className="text-gray-400 text-[10px]">{ref.referral_email}</div>
                                </td>
                                <td className="px-4 py-3 text-center">
                                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-tighter ${ref.status === 'active' ? 'bg-emerald-100 text-emerald-600' :
                            ...
                            <tr><td colSpan="3" className="px-4 py-6 text-center text-gray-400 italic">No referral data recorded.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
...
const ClinicOnboardingManager = ({ tenantId, apiCall }) => {
    ...
    if (loading) return <div className="text-gray-400 text-sm animate-pulse p-4">Loading onboarding status...</div>;
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {steps.map((step) => {
                    ...
                    return (
                        <div key={step.key} className={`p-4 rounded-2xl border transition-all flex items-center justify-between ${isComplete ? 'bg-emerald-50 border-emerald-100' : 'bg-white border-gray-100 hover:border-gray-200'}`}>
                            <div className="flex items-center gap-3">
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isComplete ? 'bg-emerald-100 text-emerald-600' : 'bg-gray-50 text-gray-400'}`}>
                                    <Icon className="w-5 h-5" />
                                </div>
                                <div>
                                    <h4 className={`text-sm font-bold ${isComplete ? 'text-emerald-800' : 'text-gray-600'}`}>{step.label}</h4>
                                    <p className="text-[10px] text-gray-400">{isComplete ? `Completed ${new Date(checklist[step.key.replace('_complete', '_date').replace('_configured', '_date').replace('_enabled', '_date')]).toLocaleDateString()}` : 'Pending setup'}</p>
                                </div>
                            </div>
                            <button
                                onClick={() => handleToggleChecklist(step.key, !isComplete)}
                                className={`p-2 rounded-lg transition-all ${isComplete ? 'text-emerald-500 hover:bg-emerald-100' : 'text-gray-400 hover:bg-gray-50'}`}
                            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
                ...
                <div className="space-y-4">
                    ...
                    {faxNumbers.length === 0 ? (
                        <div className="text-center py-6 bg-gray-50/50 rounded-2xl border border-dashed border-gray-200 text-xs text-gray-400">
                            No fax numbers assigned yet.
                        </div>
                    ) : (
                        <div className="space-y-2">
                            {faxNumbers.map(fax => (
                                <div key={fax.id} className="flex items-center justify-between p-3 bg-white border border-gray-100 rounded-xl hover:shadow-sm transition-all">
                                    <div className="flex items-center gap-3">
                                        <div className="text-indigo-500 bg-indigo-50 p-2 rounded-lg">
                                            <Activity className="w-4 h-4" />
                                        </div>
                                        <div>
                                            <p className="text-sm font-bold text-gray-700">{fax.phone_number}</p>
                                            <p className="text-[10px] text-gray-400 font-medium uppercase">{fax.label || 'Direct Inward Dialing'}</p>
                                        </div>
                                    </div>
                                    <button onClick={() => handleDeleteFax(fax.id)} className="text-gray-400 hover:text-red-500 p-2 transition-colors">
                    ...
                    {labInterfaces.length === 0 ? (
                        ...
                        <div className="space-y-2">
                            {labInterfaces.map(lab => (
                                <div key={lab.id} className="p-3 bg-white border border-gray-100 rounded-xl hover:shadow-sm transition-all">
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center gap-2">
                                            <span className={`w-2 h-2 rounded-full ${lab.status === 'active' ? 'bg-emerald-500' : 'bg-gray-200'}`}></span>
                                            <span className="text-xs font-bold text-gray-800">{lab.lab_name}</span>
                                        </div>
                                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase ${lab.status === 'active' ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-50 text-gray-500'}`}>
                                            {lab.status}
                                        </span>
                                    </div>
                                    <div className="grid grid-cols-2 gap-2 text-[10px]">
                                        <div className="text-gray-500">Facility ID: <span className="font-mono text-gray-800">{lab.facility_id || 'N/A'}</span></div>
                                        <div className="text-gray-500 text-right">Acct: <span className="font-mono text-gray-800">{lab.account_number || 'N/A'}</span></div>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            {showFaxModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-50/40 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl border border-white">
                        <h3 className="text-xl font-bold text-gray-800 mb-6">Assign Fax Number</h3>
                        <div className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none focus:border-indigo-500 transition-all"
                                />
                            </div>
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Internal Label</label>
                                <input
                                    type="text"
                        ...
                        <div className="flex gap-3 mt-8">
                            <button onClick={() => setShowFaxModal(false)} className="flex-1 py-3 text-gray-500 font-bold hover:bg-gray-50 rounded-xl transition-all">Cancel</button>
                            <button
            ...
            {showLabModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-50/40 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl border border-white">
                        <h3 className="text-xl font-bold text-gray-800 mb-6">Link Lab Interface</h3>
                        <div className="space-y-4">
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Lab Provider</label>
                                <select
                                    value={newLab.labName}
                                    onChange={(e) => setNewLab({ ...newLab, labName: e.target.value })}
                                    className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none focus:border-blue-500 transition-all"
                                >
                            ...
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Facility Identifier (MSH-4)</label>
                                <input
                                    type="text"
                                    placeholder="QUEST_FL_123"
                                    value={newLab.facilityId}
                                    onChange={(e) => setNewLab({ ...newLab, facilityId: e.target.value })}
                                    className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-mono outline-none focus:border-blue-500 transition-all"
                                />
                                <p className="text-[9px] text-gray-400 mt-1 pl-1">ID used in HL7 messages to route to this tenant</p>
                            </div>
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Lab Account #</label>
                                <input
                                    type="text"
                        ...
                        <div className="flex gap-3 mt-8">
                            <button onClick={() => setShowLabModal(false)} className="flex-1 py-3 text-gray-500 font-bold hover:bg-gray-50 rounded-xl transition-all">Cancel</button>
                            <button
                                disabled={actionLoading}
...
const ClinicFeatureManager = ({ clinicId, currentFeatures, apiCall, onUpdate }) => {
    ...
    return (
        <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 space-y-4">
            ...
            {features.map((f) => (
                <div key={f.key} className="flex items-center justify-between group">
                    ...
                    <button
                        ...
                        disabled={updating}
                        className={`w-11 h-5.5 rounded-full p-1 transition-all duration-300 ease-in-out ${currentFeatures?.[f.key] ? 'bg-emerald-500 shadow-sm shadow-emerald-500/30' : 'bg-gray-200'}`}
                    >
                        <div
            ...
            <p className="text-[10px] text-gray-400 italic mt-2 leading-tight">These toggles control access to specific clinical integrations regardless of subscription tier.</p>
        </div>
    );
};
...
const PlatformAuditTrail = ({ clinicId, apiCall }) => {
    ...
    if (loading) return <div className="text-gray-400 text-sm animate-pulse p-4">Fetching audit trail...</div>;

    return (
        <div className="space-y-3">
            {logs.length === 0 ? (
                <div className="text-center py-8 text-gray-400 italic text-sm">No platform audit logs found for this clinic.</div>
            ) : (
                logs.map((log) => (
                    <div key={log.id} className="p-3 bg-white rounded-xl border border-gray-100 flex items-start gap-3 text-xs">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between mb-0.5">
                                <span className="font-bold text-gray-800 uppercase tracking-tight">{log.action.replace(/_/g, ' ')}</span>
                                <span className="text-[10px] text-gray-400 font-mono">{new Date(log.created_at).toLocaleString()}</span>
                            </div>
                            <div className="text-gray-500 font-mono truncate bg-gray-50 p-1 rounded">
...
const ClinicDunningLog = ({ clinicId, apiCall }) => {
    ...
    if (loading) return <div className="text-gray-400 text-xs animate-pulse p-3">Loading logs...</div>;

    return (
        <>
            {selectedEmail && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-50/50 backdrop-blur-sm" onClick={() => setSelectedEmail(null)}>
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
                        <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                            <div>
                                <h3 className="font-bold text-gray-800 text-sm">{selectedEmail.subject}</h3>
                                <p className="text-xs text-gray-500">Sent to: {selectedEmail.recipient}</p>
                            </div>
                            <button onClick={() => setSelectedEmail(null)} className="p-1 hover:bg-gray-100 rounded-full"><X className="w-4 h-4 text-gray-500" /></button>
                        </div>
            ...
            <table className="w-full text-[11px]">
                <thead>
                    <tr className="text-[9px] text-gray-400 uppercase tracking-wider border-b border-slate-50 bg-gray-50/50">
                ...
                <tbody className="divide-y divide-gray-50">
                    {logs.length > 0 ? logs.map((log) => {
                        ...
                        return (
                            <tr key={log.id} className="hover:bg-gray-50/50 transition-colors group">
                                ...
                                <td className="px-3 py-3 text-center align-top">
                                    <div className="flex items-center justify-center gap-1.5 text-[10px] font-medium text-gray-600 bg-white border border-gray-100 px-2 py-0.5 rounded shadow-sm">
                                        <span className="text-gray-400">P{log.previous_phase}</span>
                                ...
                                <td className="px-3 py-3 text-gray-600 text-[10px] leading-relaxed align-top">
                                    <div className="font-medium text-gray-800 mb-0.5 flex items-center gap-2">
                                        {detailText}
                                        {isEmail && bodyHtml && (
                                            <button
                                    ...
                                    {isEmail && recipient && (
                                        <div className="text-[9px] text-gray-400">To: {recipient}</div>
                                    )}
                                    {triggeredBy && !isEmail && (
                                        <div className="text-[9px] text-gray-400 flex items-center gap-1">
                                            <span className="w-1 h-1 rounded-full bg-gray-200"></span>
                                            Triggered by: <span className="font-semibold text-gray-500">{triggeredBy}</span>
                                        </div>
                                    )}
                                </td>
                            </tr>
                        ...
                        <tr><td colSpan="4" className="px-3 py-4 text-center text-gray-400 italic text-[10px]">No logs recorded.</td></tr>
                    )}
                </tbody>
            </table>
        </>
...
const ClinicBillingStatus = ({ clinicId, apiCall }) => {
    ...
    if (loading) return <div className="text-gray-400 text-xs animate-pulse p-3">Loading billing...</div>;
    if (!billing || !billing.clinic) return <div className="text-[10px] text-gray-400 italic p-4 text-center">No subscription metadata found for this clinic.</div>;
    ...
    return (
        <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="text-xs text-gray-500">
                Revenue: <span className="font-semibold text-gray-700">${totals.totalRevenueDollars}</span>
            </span>

            <span className="text-xs text-gray-400">·</span>

            <div className="flex items-center gap-2">
                <span className="text-[10px] text-gray-400 uppercase font-semibold">Dunning Override:</span>
                <button
                    onClick={async () => {
                    ...
                    className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase border transition-colors ${clinic.billing_manual_override ? 'bg-amber-100 text-amber-700 border-amber-200' : 'bg-gray-50 text-gray-400 border-gray-200'}`}
                >
    ...
    return (
        <div className="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden mb-6">
            ...
            >
                ...
                <div className={`p-1.5 rounded-lg border border-gray-100 bg-white transition-transform ${isOpen ? 'rotate-180' : ''}`}>
                    <ChevronRight className="w-4 h-4 text-gray-400 rotate-90" />
                </div>
            </button>
            {isOpen && (
                <div className="px-6 pb-6 animate-in slide-in-from-top-2 duration-200">
                    <div className="h-px bg-gray-50 mb-6 -mx-6" />
                    {children}
                </div>
            )}
        </div>
...
const PlatformAdminClinicDetails = () => {
    ...
    return (
        <div className="min-h-screen bg-white flex flex-col lg:flex-row">
            ...
            <aside className="w-full lg:w-64 bg-white border-r border-gray-200 shrink-0 lg:h-screen lg:sticky lg:top-0 transition-all duration-300">
                <div className="p-6">
                    <button
                        onClick={() => navigate('/platform-admin/clinics')}
                        className="flex items-center gap-2 text-gray-400 hover:text-gray-600 mb-8 transition-colors text-xs font-semibold group uppercase tracking-widest"
                    >
                ...
                <div className="mt-auto p-6 border-t border-slate-50">
                    <div className="p-4 bg-red-50/50 rounded-2xl border border-red-100 mb-4">
                        <h3 className="text-[10px] font-bold text-red-900 uppercase tracking-wider mb-2 flex items-center gap-2">
                    ...
                    <p className="text-[9px] text-gray-400 font-mono text-center">v{clinic.emr_version} • {clinic.slug}</p>
                </div>
            </aside>
            ...
            <main className="flex-1 min-w-0 h-screen overflow-y-auto scroll-smooth">
                {/* Header Strip */}
                <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-gray-100 px-8 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm">
                    <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="px-4 py-2 bg-gray-50 text-white rounded-xl text-xs font-bold shadow-lg shadow-slate-200 hover:bg-gray-100 transition-all flex items-center gap-2 active:scale-95 disabled:opacity-50"
                        >
                            <Zap className={`w-3.5 h-3.5 ${impersonating ? 'animate-pulse' : ''}`} />
                            {impersonating ? 'Preparing...' : 'Platform Takeover'}
                        </button>
                        <div className="h-8 w-px bg-gray-50 hidden md:block" />
                        <div className="flex items-center gap-1.5 p-1 bg-gray-50 rounded-lg border border-gray-100">
                            {[
                                ...
                                { status: 'deactivated', icon: XCircle, color: 'text-gray-400' }
                            ].map((s) => (
                                <button
                                    key={s.status}
                                    onClick={() => handleStatusChange(s.status)}
                                    disabled={statusUpdating || clinic.status === s.status}
                                    className={`p-1.5 rounded-md transition-all ${clinic.status === s.status ? 'bg-white shadow-sm ring-1 ring-gray-200/50' : 'opacity-40 hover:opacity-100'}`}
                                    title={`Set as ${s.status}`}
                                >
                                    <s.icon className={`w-3.5 h-3.5 ${clinic.status === s.status ? s.color : 'text-gray-400'}`} />
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                <div className="p-8 pb-20">
                    <div className="max-w-5xl mx-auto">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="p-6 bg-white border border-gray-100 rounded-3xl shadow-sm">
                                        <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4 font-mono">Clinic Entity Meta</h3>
                                        <div className="space-y-4">
                                            <div className="space-y-1">
                                                <label className="text-[10px] font-bold text-gray-400 uppercase">Schema (Tenant ID)</label>
                                                <div className="flex items-center gap-2">
                                                    <input
                                                        value={clinic.slug}
                                                        readOnly
                                                        className="flex-1 bg-gray-50 border border-gray-100 rounded-xl px-3 py-2 text-xs font-mono font-bold text-gray-500"
                                                    />
                                                    <button onClick={() => navigator.clipboard.writeText(clinic.slug)} className="p-2 hover:bg-gray-50 rounded-lg transition-colors">
                                        ...
                                        <div className="space-y-2">
                                            {recent_payments?.map((payment, i) => (
                                                <div key={i} className="flex items-center justify-between p-3 bg-gray-50 rounded-2xl border border-gray-100">
                                                    <div>
                                                        <p className="text-xs font-bold text-gray-700">${payment.amount}</p>
                                                        <p className="text-[10px] text-gray-400 font-mono">{new Date(payment.created_at).toLocaleDateString()}</p>
                                                    </div>
                                                    <span className={`px-2 py-0.5 rounded-full text-[8px] font-bold uppercase ${payment.status === 'succeeded' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}>
                                                        {payment.status}
                                                    </span>
                                                </div>
                                            ))}
                                            {(!recent_payments || recent_payments.length === 0) && <p className="text-xs text-gray-400 italic text-center py-4">No payment history.</p>}
                                        </div>
                                    </CollapsibleCard>
                                    ...
                                    <CollapsibleCard title="Clinical Safety Kill-Switches" icon={AlertCircle}>
                                        <div className="space-y-4">
                                            {[
                                                ...
                                                <div key={sw.key} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 group transition-all hover:border-gray-200">
                                                    <div>
                                                        <span className="text-xs font-bold text-gray-700 block">{sw.label}</span>
                                                        <span className="text-[10px] text-gray-400 font-medium uppercase tracking-tighter">{sw.desc}</span>
                                                    </div>
                                                    <button
                                                        onClick={() => handleControlChange({ [sw.key]: !clinic[sw.key] })}
                                                        disabled={controlsUpdating}
                                                        className={`w-12 h-6 rounded-full p-1 transition-all duration-300 ${clinic[sw.key] ? sw.color : 'bg-gray-200'}`}
                                                    >
                        ...
                        {activeTab === 'integrations' && (
                            <div className="bg-white border border-gray-100 rounded-3xl p-8 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-300">
                                <h2 className="text-sm font-bold text-gray-800 uppercase tracking-wider flex items-center gap-2 mb-8">
