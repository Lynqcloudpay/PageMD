...
const FlagAcknowledgmentModal = ({ flags, onAcknowledged }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-gray-50/60 backdrop-blur-sm">
            <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="bg-red-600 p-6 text-white text-center">
                    ...
                    <h2 className="text-xl font-bold uppercase tracking-tight">Security & Clinical Alerts</h2>
                ...
                <div className="p-6 space-y-4 max-h-[40vh] overflow-y-auto">
                    {flags.map(flag => (
                        <div key={flag.id} className="p-4 bg-red-50 border border-red-100 rounded-2xl flex gap-3">
                            ...
                            <div>
                                <h4 className="font-bold text-red-900 text-sm uppercase tracking-tight">{flag.label}</h4>
                ...
                <div className="p-6 border-t bg-gray-50 flex flex-col gap-3">
                    <button
                        ...
                        disabled={loading}
                        className="w-full py-4 bg-red-600 text-white rounded-2xl font-bold text-sm uppercase tracking-widest hover:bg-red-700 transition-all shadow-lg shadow-red-200 flex items-center justify-center gap-2 active:scale-[0.98]"
                    >
                        {loading ? 'Processing...' : (
                            <>
                                <CheckCircle size={18} />
