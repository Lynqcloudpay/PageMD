...
const Layout = ({ children }) => {
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#F0F4F5] transition-colors">
            ...
            <div className="flex flex-1">
                ...
                <aside
                    className={cn(
                        ...
                        sidebarCollapsed ? 'w-[4.5rem]' : 'w-[14rem]'
                    )}
                >
                    <div className="flex flex-col h-full bg-[#003133] border-r border-black/10">
                        ...
                        <div className={cn(
                            ...
                            {!sidebarCollapsed ? (
                                <>
                                    <Link to="/dashboard" className="flex items-center group">
                                        <img
                                            src="/logo.png"
                                            alt="PageMD Logo"
                                            className="h-7 w-auto object-contain max-w-[100px] brightness-0 invert opacity-90 group-hover:opacity-100 transition-opacity"
                                            onError={(e) => { e.target.style.display = 'none'; }}
                                        />
                                    </Link>
                                    <button
                                        onClick={() => setSidebarCollapsed(true)}
                                        className="p-1.5 rounded-md text-white/30 hover:text-white/60 hover:bg-white/10 transition-all"
                                ...
                                <button
                                    ...
                                    className="p-1 rounded-md hover:bg-gray-100 transition-all group"
                                ...
                                >
                                    ...
                                    />
                                    <div className="hidden w-6 h-6 rounded-md bg-gray-100 flex items-center justify-center text-gray-600 text-[10px] font-semibold">P</div>
                                </button>
                            )}
                        </div>
                        ...
                        {!sidebarCollapsed && (
                            <div className="px-2.5 pt-3 pb-1">
                                <button
                                    ...
                                    className="w-full flex items-center gap-2 px-3 py-2 rounded-lg bg-black/10 text-white/40 text-[11px] hover:bg-black/20 hover:text-white/60 transition-colors border border-white/5"
                                >
                                    <Search className="w-3 h-3" strokeWidth={1.8} />
                                    <span>Search...</span>
                                    <kbd className="ml-auto text-[9px] font-mono text-gray-400 bg-gray-200/60 px-1 py-0.5 rounded">⌘K</kbd>
                                </button>
                            </div>
                        )}
                        ...
                        <div className="border-t border-white/5 p-3 bg-black/5">
                            {user && (
                                <div className="flex items-center gap-2">
                                    ...
                                    >
                                        <div className="w-8 h-8 rounded-lg bg-cyan-600 flex items-center justify-center text-white text-[11px] font-bold shadow-sm flex-shrink-0">
                                            {(user.firstName?.[0] || 'U') + (user.lastName?.[0] || '')}
                                        </div>
                                        {!sidebarCollapsed && (
                                            <div className="text-left flex-1 min-w-0">
                                                <div className="text-[12px] font-bold text-white/90 leading-tight truncate">{user.firstName} {user.lastName}</div>
                                                <div className="text-[10px] text-white/40 font-medium truncate uppercase tracking-wider">{user.role_name || user.role || 'User'}</div>
                                            </div>
                                        )}
                                    </button>
                                    {!sidebarCollapsed && (
                ...
                <main
                    className={cn(
                        ...
                        sidebarCollapsed ? "ml-[4.5rem]" : "ml-[14rem]"
                    )}
                >
                    {/* Header strip */}
                    ...
                    {/* Page Content */}
                    <div className="flex-1 overflow-y-auto min-h-0 relative h-full bg-[#F0F4F5]">
